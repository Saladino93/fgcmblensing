...
poles = [1, -1]
shifts = list(range(4))
ras, decs, M_200 = np.array([]), np.array([]), np.array([])
for p in poles:
    for shift in shifts:
...
np.save(source_dir/'counts', counts)
hp.write_map(source_dir/'counts', counts)

hp.write_map(source_dir/'counts_alm', counts_alm)
...
#np.save(source_dir/'maschera', maschera)
#hp.write_map(source_dir/'maschera', maschera)

hp.mollview(maschera)
plt.savefig('maschera.png')
