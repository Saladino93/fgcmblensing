...
#plens_directory = os.environ['PLENS']
plens_directory = '/global/cscratch1/sd/omard/PLENS/'
TEMP =  os.path.join(plens_directory, 'temp', 'idealized_example_sehgal')
...
lmax_ivf = 3000 #LMAX OF RECONSTRUCTION
...
nside = 4096 # Healpix resolution of the data and sims.
...
transf = transf*0.+1. #NOTE: I CHANGE TRANSFER FUNCTION HERE
...
#GETTING FIDUCIAL SPECTRA FOR RECONSTRUCTION
...
#sims = maps_utils.sim_lib_omar(maps.cmb_maps_nlev_sehgal(planck2018_sims.cmb_len_ffp10(), transf, nlev_t, nlev_p, nside,
#                            pix_lib_phas=pix_phas))

sims = maps_utils.sim_lib_omar(maps.cmb_maps_noiseless_sehgal(planck2018_sims.cmb_len_ffp10(), transf, nside))
...
te = cl_len['te']
te = None
qlms_dd = qest.library_sepTP(os.path.join(TEMP, 'qlms_dd'), ivfs, ivfs,   te, nside, lmax_qlm=lmax_qlm)
qlms_ds = qest.library_sepTP(os.path.join(TEMP, 'qlms_ds'), ivfs, ivfs_d, te, nside, lmax_qlm=lmax_qlm)
qlms_ss = qest.library_sepTP(os.path.join(TEMP, 'qlms_ss'), ivfs, ivfs_s, te, nside, lmax_qlm=lmax_qlm)
