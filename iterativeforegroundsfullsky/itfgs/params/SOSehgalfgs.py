...
Here we generate Gaussian data.
...
include_fgs_power = True

baseSehgal = opj(os.environ['SCRATCH'], 'SKYSIMS/SEHGALSIMS/')
#baseSehgal = opj(os.environ['SCRATCH'], 'SehgalSims')

SimsShegalDict = {}
SimsShegalDict['kappa'] = lambda idx: opj(baseSehgal, 'healpix_4096_KappaeffLSStoCMBfullsky.fits')

SimsShegalDict[f'fg'] = lambda idx: opj(baseSehgal, ['GAUSSIANMAPS', 'MAPS'][idx], 'sum.fits')
...
class SehgalSim(sims_postborn):
    ...
    def get_sim_tlm(self, idx):
        ...
        idxcmb = 0
        print(f'Alternative tlm!! idxcmb {idxcmb}')
        fname = os.path.join(self.lib_dir, 'sim_%04d_tlm.fits' % idxcmb)

        fnamefg = os.path.join(self.lib_dir, 'sim_fg_%04d_tlm.fits' % idx)

        super().get_sim_tlm(idx) # This will generate the CMB tlm if it does not exist

        if not os.path.exists(fnamefg):
            get_element = lambda nome: hp.map2alm(hp.read_map(nome), lmax = self.lmax, iter = 0)
            somma = get_element(self.sims[f'{self.fgkey}'](idx))
            hp.write_alm(fnamefg, somma)

        return hp.read_alm(fname)+hp.read_alm(fnamefg)

    



suffix = 'SOSehgalfgsCORR' # descriptor to distinguish this parfile from others...
...
ells, lcmb, tsz, ksz, tsz_cib, radio, cib, dust, nl145, totalcmb, totalnoisecmb = allelementstosave.T
fgs = (tsz+ksz+radio+cib+dust+tsz_cib)*include_fgs_power
