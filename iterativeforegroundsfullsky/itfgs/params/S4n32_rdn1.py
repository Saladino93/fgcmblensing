...
def get_info(caso: str) -> tuple:
    ...
    if caso == casorand:
        ...
        #odd indices from 400 to 500
        indices = np.arange(400, 510, 2)
        dictionary = {}
        for ind in indices:
            dictionary[ind+1] = ind
            dictionary[ind] = ind

        SimsShegalDict = {}
        SimsShegalDict['kappa'] = lambda idx: opj(baseSehgal, f'bornGaussian/born_kappa_gauss_alm_{dictionary[idx]}.fits')
        ...
        SimsShegalDict = {}
        SimsShegalDict['kappa'] = lambda idx: opj(baseSehgal, f'bornGaussian/rotation_kappa_gauss_alm_{idx}.fits')
...
def get_all(case: str, case_specific: str, sim_index: int):
    ...
    #case_specific = ""
    suffixCMB_data, suffixCMBPhas_data, suffixLensing_data, SimsShegalDict_data, extra_tlm_data = get_info(case_specific)
    TEMPDATA = opj(os.environ['SCRATCH'], main_dir, suffixLensing_data, 'lenscarfrecs')
    SIMDIR_data = opj(os.environ['SCRATCH'], main_dir, suffixCMB_data, 'cmbs')

    names = ['']
    SimsShegalDict_data[0] = [lambda idx: opj(baseSehgal, nome) for nome in names]
    ...
    if case_specific == "gauss":
        TEMP_RDN0 = "/users/odarwish/scratch/n32OFFICIAL/S4GiulioN15120/"
    #elif case_specific == "rand":
    #    TEMP_RDN0 = "/users/odarwish/scratch/n32OFFICIAL/S4GiulioRandRDN05120/"

    X = sim_index
    directory = pathlib.Path(TEMP_RDN0)#/f"{X}"
    directory.mkdir(parents=True, exist_ok=True)

    libdir_iterators = lambda qe_key, simidx, version: opj(directory,'%s_sim%04d'%(qe_key, simidx) + version)
    ...
    zero_noise = False
    fixed_noise_index = None #this will allow to have always the same experimental noise realization
    lmax_cmb = 4096
    dlmax = 1024
    ...
    sims_cmb_len = SehgalSim(sims = SimsShegalDict_data, lib_dir = SIMDIR_data, lmax_cmb = lmax_cmb, cls_unl = cls_unl, dlmax = dlmax, lmin_dlm = 2, lib_pha = libPHASCMB, extra_tlm = extra_tlm)
    sims_data = simsit.cmb_maps_nlev_sehgal(sims_cmb_len = sims_cmb_len, cl_transf = transf_dat, 
                                    nlev_t = nlev_t, nlev_p = nlev_p, nside = nside, pix_lib_phas = pix_phas, zero_noise = zero_noise, fixed_noise_index = fixed_noise_index)

    #these are my simulations, so in my case the Gaussian born simulations
    ivfs   = filt_simple.library_fullsky_sepTP(opj(TEMP, 'ivfs'), sims, nside, transf_d, cls_len, ftl, fel, fbl, cache=True)

    sim_set_indices = np.arange(400, 510)

    #this is the data map
    #ivfs_d = filt_simple.library_fullsky_sepTP(opj(TEMPDATA, 'ivfs'), sims_data, nside, transf_d, cls_len, ftl, fel, fbl, cache=True)
    #I then add here this shuffling library to get always the same exact simulation for the data map

    #ds_dict = {k : X for k in sim_set_indices} #my reference is the simulation number X
    #ivfs_d = filt_util.library_shuffle(ivfs_d, ds_dict)

    ss_dict = { k : k+1 for k in sim_set_indices} #from 200 to 300 they all have independent noise realizations
    ...
    qlms_dd = qest.library_sepTP(os.path.join(directory, 'qlms_dd'), ivfs, ivfs,   cls_len['te'], nside, lmax_qlm=lmax_qlm)
    #qlms_ds = qest.library_sepTP(os.path.join(directory, 'qlms_ds'), ivfs, ivfs_d, cls_len['te'], nside, lmax_qlm=lmax_qlm)
    qlms_ss = qest.library_sepTP(os.path.join(directory, 'qlms_ss'), ivfs, ivfs_s, cls_len['te'], nside, lmax_qlm=lmax_qlm)
    ...
    def get_itlib(k:str, simidx:int, version:str, cg_tol:float, epsilon=1e-5):
        ...
        if not os.path.exists(path_plm0):
            ...
            #plm0_ds = qlms_ds.get_sim_qlm(k, int(simidx))  #Unormalized quadratic estimate:
            plm0_ss = qlms_ss.get_sim_qlm(k, int(simidx))  #Unormalized quadratic estimate:
            ...
            casenames = ["sim", "sim_ss"]#, "sim_ds", "sim_ss"]

            for i, p_temp in enumerate([plm0, plm0_ss]):#, plm0_ds, plm0_ss

                path_plm0 = opj(libdir_iterator, f'{casenames[i]}_phi_plm_it000.npy')
                path_plm0_QE_norm = opj(libdir_iterator, f'{casenames[i]}_normalized_phi_plm_it000.npy')

                plm0_ = alm_copy(p_temp,  None, lmax_qlm, mmax_qlm) # Just in case the QE and MAP mmax'es were not consistent
                almxfl(plm0_, utils.cli(R), mmax_qlm, True) # Normalized QE
                np.save(path_plm0_QE_norm, plm0_)
                np.savetxt(opj(libdir_iterator, "WF.txt"), WF)
                almxfl(plm0_, WF, mmax_qlm, True)           # Wiener-filter QE
                almxfl(plm0_, cpp > 0, mmax_qlm, True)
                np.save(path_plm0, plm0_)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-case_specific', dest='case_specific', type=str, default="", help='case specific')
    parser.add_argument('-sim_index', dest='sim_index', type=int, default=0, help='sim index')
    ...
    get_itlib, libdir_iterators, chain_descrs, lmax_unl, _, _ = get_all(args.case, args.case_specific, args.sim_index)
