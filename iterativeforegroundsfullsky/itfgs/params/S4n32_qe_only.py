...
def get_info(caso: str) -> tuple:
    ...
    if caso == casorand:
        ...
        suffixCMB = suffix+'BornRand5120_3000_noiseless'
        suffixCMBPhas = suffix
        suffixLensing = suffix+'BornRand5120_3000_noiseless'

        SimsShegalDict = {}
        SimsShegalDict['kappa'] = lambda idx: opj(baseSehgal, 'map0_kappa_ecp262_dmn2_lmax8000_first_randomized_alm.fits')

    elif caso == casogauss:
        suffixCMB = suffix+'BornGauss5120_3000_noiseless'
        suffixCMBPhas = suffix
        suffixLensing = suffix+'BornGauss5120_3000_noiseless'

        SimsShegalDict = {}
        SimsShegalDict['kappa'] = lambda idx: opj(baseSehgal, f'bornGaussian/born_kappa_gauss_alm_{idx}.fits')

    elif caso == casostd:
        suffixCMB = suffix+'Born5120_3000_noiseless'
        suffixCMBPhas = suffix
        suffixLensing = suffix+'Born5120_3000_noiseless'
...
if __name__ == '__main__':
    ...
    jobs = np.arange(args.imin, args.imax+1)
    for idx in jobs:
        get_itlib(args.k, idx, args.v)
