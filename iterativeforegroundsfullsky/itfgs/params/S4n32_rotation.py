...
class SehgalSim(sims_postborn):
    ...
    def get_sim_omega_alm(self, idx, verbose: bool = True):
        if verbose:
            print('Getting special omega alm!')
        nome = self.sims[self.omegakey](idx)
        return hp.read_alm(nome)
...
def get_info(caso: str) -> tuple:
    ...
    if caso == casorand:
        ...
        suffixCMB = suffix+'RotGauss5120_3fields'
        suffixCMBPhas = suffix
        suffixLensing = suffix+'RotGauss5120_3fields'

        SimsShegalDict = {}
        SimsShegalDict['omega'] = lambda idx: opj(baseSehgal, f'rotationGaussian/rotation_kappa_gauss_alm_{idx}.fits')
        SimsShegalDict['kappa'] = lambda idx: opj(baseSehgal, f'postbornGaussian/postborn_kappa_gauss_alm_{idx}.fits')

    elif caso == casorot:

        suffixCMB = suffix+'Rot5120_3fields'
        suffixCMBPhas = suffix
        suffixLensing = suffix+'Rot5120_3fields'

        SimsShegalDict = {}
        SimsShegalDict['omega'] = lambda idx: opj(baseSehgal, 'map0_rotation_ecp262_dmn2_lmax8000_alm.fits')
        ...
        SimsShegalDict = {}
        SimsShegalDict['omega'] = lambda idx: opj(baseSehgal, 'map0_rotation_ecp262_dmn2_lmax8000_randomized_alm.fits')
...
def get_all(case: str):
    ...
    #SIMDIR_with_curl = opj(os.environ['SCRATCH'], main_dir, suffixCMB, 'cmbs_with_curl')
    #lib_dir_CMB = opj(os.environ['SCRATCH'], main_dir, suffixCMBPhas, 'cmbs_with_curl') #this is where I store phas, if already computed
    lib_dir_CMB = opj(os.environ['SCRATCH'], main_dir, suffixCMBPhas, 'cmbs')
    ...
    print("SIMDIR: ", SIMDIR)
    #print("SIMDIR_with_curl: ", SIMDIR_with_curl)
    ...
    sims_cmb_len = SehgalSim(sims = SimsShegalDict, wcurl = True, lib_dir = SIMDIR, lmax_cmb = lmax_cmb, cls_unl = cls_unl_wcurl, dlmax = dlmax, lmin_dlm = 2, lib_pha = libPHASCMB, extra_tlm = extra_tlm)
    ...
    fal["ee"] = fel
    fal["bb"] = fbl
    resplib = qresp.resp_lib_simple(opj(TEMP, 'qlms_dd_curl'), lmax_ivf, cls_weight = cls_grad, cls_cmb = cls_len, fal = fal, lmax_qlm = lmax_qlm)
    qlms_dd = qest.library_sepTP(opj(TEMP, 'qlms_dd_curl'), ivfs, ivfs,   cls_len['te'], nside, lmax_qlm=lmax_qlm, resplib = resplib)
    qcls_dd = qecl.library(opj(TEMP, 'qcls_dd_curl'), qlms_dd, qlms_dd, mc_sims_bias)
    # -------------------------
    ...
    def get_itlib(k:str, simidx:int, version:str, cg_tol:float, epsilon=1e-5):
        ...
        mf0_p = qlms_dd.get_sim_qlm_mf('p', mf_sims)  # Mean-field to subtract on the first iteration:
        mf0_o = qlms_dd.get_sim_qlm_mf('x' + k[1:], mf_sims)
        ...
        path_plm0 = opj(libdir_iterator, 'start_phi_plm_it000.npy')
        path_plm0_QE_norm = opj(libdir_iterator, 'normalized_phi_plm_it000.npy')

        path_Olm0 = opj(libdir_iterator, 'phi_olm_it000.npy')
        path_Olm0_QE_norm = opj(libdir_iterator, 'normalized_omega_olm_it000.npy')
        ...
        if not os.path.exists(path_plm0):
            ...
            olm0 = qlms_dd.get_sim_qlm('x' + k[1:], int(simidx)) #- mf0_o
            ...
            Rpp, Roo = qresp.get_response(k, lmax_ivf, 'p', cls_weight = cls_len, cls_cmb = cls_grad, fal = fal, lmax_qlm=lmax_qlm)[0:2]
            np.savetxt(opj(libdir_iterator, "R.txt"), np.c_[Rpp, Roo])
            # Isotropic Wiener-filter (here assuming for simplicity N0 ~ 1/R)

            WF = cpp * utils.cli(cpp + utils.cli(Rpp))
            ...
            np.save(path_Olm0_QE_norm, olm0)
            ...
            np.save(path_Olm0, olm0)

    
        plm0 = np.load(path_plm0)
        olm0 = np.load(path_Olm0)

        #NOTE: UNLENSED RESPONSE HERE
        Rpp_unl, Roo_unl = qresp.get_response(k, lmax_ivf, 'p', cls_unl, cls_unl,  {'e': fel_unl, 'b': fbl_unl, 't':ftl_unl}, lmax_qlm=lmax_qlm)[0:2]
        ...
        if k in ['ptt', "ptt_bh_s"]:
            ...
            filtr = alm_filter_ee_wl(nlev_p, ffi, transf_elm, (lmax_unl, mmax_unl), (lmax_ivf, mmax_ivf),
                                    transf_b=transf_blm, nlev_b=nlev_p, wee = True)
        ...
        iterator = iterator_cstmf_wcurl(libdir_iterator, 'p', [(lmax_qlm, mmax_qlm), (lmax_qlm, mmax_qlm)], datmaps,
                [plm0, olm0], [mf0_p, mf0_o], [Rpp_unl, Roo_unl], [cpp, coo], ('p', 'x'), cls_unl, filtr, k_geom, chain_descrs(lmax_unl, cg_tol), stepper
                ,wflm0 = wflm0)
        return iterator
