...
from delensalot.core.opfilt.MAP_opfilt_iso_t_mf import alm_filter_nlev_wl as alm_filter_tt_wl
...
def get_all(case: str):
    ...
    zero_noise = False
    fixed_noise_index = 0 #0 #this will allow to have always the same experimental noise realization
    ...
    def get_itlib(k:str, simidx:int, version:str, cg_tol:float, epsilon=1e-5):
        ...
        libPHASCMB_mf = phas.lib_phas(os.path.join(lib_dir_CMB, 'phas_cmb_mf'), 3, lmax_cmb + dlmax)
        libPHASNOISE_mf = phas.lib_phas(os.path.join(lib_dir_CMB, 'phas_noise_mf'), 3, lmax_cmb + dlmax)

        mc_sims = [50, 50, 50, 50, 50, 50, 50, 50] #uses 20 sims for each it
        sub_nolensing = True
        iterator = scarf_iterator.iterator_simf_mcs(libdir_iterator, 'p', (lmax_qlm, mmax_qlm), datmaps,
                plm0, mf_key = 1, pp_h0 = R_unl, cpp_prior = cpp, cls_filt = cls_unl, ninv_filt = filtr, k_geom = k_geom, chain_descr = chain_descrs(lmax_unl, cg_tol), stepper = stepper
                ,mc_sims = mc_sims, sub_nolensing = sub_nolensing, mf_cmb_phas = libPHASCMB_mf, mf_noise_phas = libPHASNOISE_mf, wflm0 = wflm0)
                #sht_job = sht_job, sht_threads = tr, verbosity = 0, epsilon = epsilon)
        return iterator
